...
public class Draggable : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        mainCamPosition = targetCamera.transform.position;
    }
    ...
    {
        Ray ray = targetCamera.ScreenPointToRay(Input.mousePosition);
        if (Physics.Raycast(ray, out hitFloor, Mathf.Infinity))
        {
            float h = dragAnchor.transform.position.y;

            Vector3 camToFloor = hitFloor.point - targetCamera.transform.position;
